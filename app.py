...
<style>
  ...
  /* ── Global background ── */
  .stApp {
    background: linear-gradient(135deg, #1a0000 0%, #3d0000 25%, #5c0a0a 50%, #3d0000 75%, #1a0000 100%);
    background-attachment: fixed;
  }

  /* ── Sidebar ── */
  [data-testid="stSidebar"] {
    background: linear-gradient(180deg, #0d0000 0%, #2a0000 50%, #1a0000 100%) !important;
  ...
  h1 {
    ...
    text-shadow: 0 0 40px rgba(255,100,100,0.4) !important;
  }
  ...
  [data-testid="stMetric"] {
    background: linear-gradient(135deg, rgba(80,0,0,0.6) 0%, rgba(40,0,0,0.8) 100%) !important;
...
# ── Load data ────────────────────────────────────────────────
# When running locally  → reads from CSV files
# When deployed on Streamlit Cloud → reads from Google Sheets

@st.cache_data(ttl=300)
def load_data():
    try:
        # Try to read from Google Sheets (Streamlit Cloud)
        sheet_id = st.secrets["sheets"]["sheet_id"]

        url1 = f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&sheet=anomaly_alerts"
        results = pd.read_csv(url1)
        results["anomaly"] = results["anomaly"].astype(str).str.lower() == "true"

        url2 = f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&sheet=patient_risk_summary"
        patients = pd.read_csv(url2)
        patients["any_anomaly"] = patients["any_anomaly"].astype(str).str.lower() == "true"

        # Vitals still from CSV - too large for Sheets
        ...
        st.sidebar.info("📂 Reading from local CSV files")
        return results, patients, vitals_df
